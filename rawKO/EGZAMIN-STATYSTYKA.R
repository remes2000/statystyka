...
# (ZAD 1.13)
cat("\014")
moja_lista <- list(c("Krystian, Osi�ski"),3.14,function(x) sqrt(x),seq(0.02,1,by=0.02))
moja_lista
...
barplot(table(ankieta$wynik),xlab = "odpowiedzi",ylab="Liczebno��",main="Rozk�ad empiryczny zmiennej wynik")
...
load("Centrala.RData")
x <- Centrala
data.frame(cbind(liczba_zg�osze� = table(x),prawdopodobie�stwo = prop.table(table(x))))

barplot(table(x),labx="liczba zg�osze�",laby="Liczebno��",main="Rozk�ad empiryczny liczby zg�osze� w Centrali")
barplot(prop.table(table(x)),xlab="Liczba zg�osze�",ylab="Prawdopodoie�stwo",main="Rozk�ad empiryczny liczby zg�osze� w centrali")
pie(table(x))
(mean(x$Liczba))
(median(x$Liczba))
(sd(x$Liczba))
...
a <-  function(x,na.rm = FALSE){
  if(!is.numeric(x)){
    stop("x nie jest warto�ci� numeryczn�!")
  }
...
barplot(prop.table(table(Centrala$Liczba)),
        main="Rozk��d empiryczny liczby zg�osze�",
...
counts
barplot(counts, xlab = "Liczba zg�osze� w centrali", ylab="Prawdopodobie�stwo",main="Rozk�ad empiryczny i teoretetyczny liczby zg�osze� centrali",col=c("red","blue"),legend=rownames(counts),beside=TRUE)
...
#Odpowied�: �rednia g��boko�� morza wynosi 870m
...
var(A)
var(B)
var.test(A,B,alternative = "less") #r�wno�� wariancji 
mean(A)
mean(B)
t.test(A,B,alternative = "greater", var.equal = TRUE)
# odrzucamy Ho, przyjmujemy H1
# Odpowied�: Proszek do prania A jest znacz�co lepszy on proszku do prania B
rm(list=ls())

##=======================================================
# (ZAD 5.3)
cat("\014")
# 2 pr�by zale�ne - rozk�ad normalny
#H0: mu1 == mu2
przed <- c(84, 87, 87, 90, 90, 90, 90, 93, 93, 96)
po <-    c(89, 92, 98, 95, 95, 92, 95, 92, 98, 101.)
boxplot(przed,po)
shapiro.test(przed) #normalno�� - brak podstaw do odrzucenia H0
shapiro.test(po) #normalno�� - brak podstaw do odrzucenia H0
mean(przed)
mean(po)
t.test(przed,po, alternative = "less", paired = TRUE)
# odrzucamy H0, przyjmujemy H1
# Odpowied�: Film ma znacz�cy wp�yw poprawienie podej�cia do tego typu szk�
rm(list=ls())
...
boxplot(m,k) 
shapiro.test(m) #normalno�� - brak podstaw do odrzucenia H0
shapiro.test(k) #normalno�� - brak podstaw do odrzucenia H0
var(m)
var(k)
var.test(m,k,alternative = "greater") #brak takiej samej wariancji!
# odrzucenie testu - t.studenta!
# przej�cie na test welcha!
mean(m)
mean(k)
t.test(m,k,var.equal = FALSE,alternative = "greater")
#odrzucenie H0! Przyj�cie H1
# odpowied� �redni wzorst m�czyzn jest znacz�ca wi�kszy od �redniego wzrostu kobiet
rm(list=ls())

##=======================================================
# (ZAD 5.6)
cat("\014")
# nale�y wykorzysta� odpowiedni test nieparametryczny!
...
# odrzucamy H0
#odpowied� hipnoza zmniejsza znacz�co poziom odczuwanego b�lu
...
#Odrzucamy H0 - przyjmujemy H1
#M�czy�ni maj� znacz�co wi�kszy �redni wzrost ni� kobiety
...
#odrzucamy H0 - przyjmujemy H1
# Odpowied�: w miej�cie B statystycznie znacz�co wi�cej firm korzytsa z reklam
rm(list=ls())
...
mcnemar.test(x)
#odrzucamy h0 - przyjmujemy H1
# odpowied�: Istnieje istotna r�nica w ocenie dzia�alno�ci prezydenta
rm(list=ls())
...
#odpowied�: Proporcje odpowiadaj� preferencjom klient�w
rm(list=ls())
...
chisq.test(x)
#Odrzucamy H0 - przyjmujemy H1
#Jako� produktu zale�y od metody produkcji
...
# 1.Wyznacz �rednie i zr�b wykres
